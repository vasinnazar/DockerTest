...
class Synchronizer {
    ...
    static function updateLoanRepayments($series = null, $number = null, $loan_id_1c = null, $customer_id_1c = null) {
        ...
        if (!is_null($loan_id_1c) && array_key_exists('loan', $contracts1c)) {
             ...
             */
            if (!array_key_exists('claim', $contracts1c) || empty($contracts1c['claim']['id_1c'])) {
                ...
                if (is_null($passport)) {
                    ...
                    if (array_key_exists('loan', $contracts1c) && !empty($contracts1c['loan']['id_1c']) && array_key_exists('claim', $contracts1c)) {
                        $claim = Synchronizer::addEmptyClaim($passport, $customer, $about_client, $contracts1c['loan']['id_1c']);
                        $loan = Synchronizer::updateLoan($contracts1c, $claim, $passport);
                    } else {
...
//            Synchronizer::updateOrders($loan->created_at, $claim->passport->series, $claim->passport->number, null, $loan, Carbon::now());
    ...
    static function addEmptyClaimLoan($passport, $customer, $about_client) {
        $emptySubdiv = Subdivision::where('name_id', '113')->first();
        $claim = Synchronizer::addEmptyClaim($passport, $customer, $about_client, '');
        $loan = new Loan();
...
//        if(is_null($passport)){
        $loan = Loan::where('id_1c', $contracts1c['loan']['loan_id_1c'])->first();
//        } else {
//            $loan = Loan::where('loans.id_1c', $contracts1c['loan']['loan_id_1c'])->leftJoin('claims', 'claims.id', '=', 'loans.claim_id')->where('claims.passport_id', $passport->id)->first();
//        }
        if (is_null($loan)) {
...
//        else {
//            if ($loan->created_at->gte(new Carbon(config('options.new_rules_day')))) {
//                $loantype = LoanType::find(config('options.new_rules_loantype_id'));
//            }
//        }
        $loan->loantype_id = $loantype->id;
        if (is_null($loan->id)) {
    ...
    static function removeUnreceivedRepayments($contracts1c, $loan) {
        ...
        foreach ($reps as $rep) {
            ...
            if (!$received || $duplicate) {
                //если допник или допник с комиссией и версия арма - продажная - то оставить их. 
                // а если версия арма - для должников, то удалить все лишние допники
                // сделано на случай задвоенных договоров, чтобы задолженность 
                // запрашивалась точно по последнему доку этого договора
                if (($rep->repaymentType->isDopnik() || $rep->repaymentType->isDopCommission()) && !config('app.version_type') != 'debtors' && !$duplicate) {
                    continue;
                }
...
//                if ((strstr($reason, 'Возврат процентов') !== FALSE || strstr($reason, 'Оплата процентов и основного долга') !== FALSE || strstr($item, 'Просроченные проценты') !== FALSE) && $account == '62.04') {
            ...
            if (isset($o->order_type) && in_array((string) $o->order_type, [3, 20, 21, 22])) {
