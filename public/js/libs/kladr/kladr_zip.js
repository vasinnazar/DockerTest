		...
		this.keyup(function () {
			...
			$.kladr.api({
				...
				if (obj) {
					...
					objs.push(obj);
                                        //ГРЯЗНЫЙ ХАК
                                        //В РУЧНУЮ ПРОПИСЫВАЮ КОРОТКИЕ ТИПЫ ПОЛЕЙ ЧТОБЫ ПОЛУЧАЛОСЬ "КЕМЕРОВСКАЯ ОБЛ" ВМЕСТО "КЕМЕРОВСКАЯ"
                                        for(var o in objs){
                                            objs[o].name = (objs[o].contentType != 'building') ? (objs[o].name + ' ' + objs[o].typeShort) : objs[o].name;
                                        }
                                        //
