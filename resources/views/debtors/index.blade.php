...
@section('content')
    <div class="row">
        <div class="col-xs-12">
            <div class='pull-right btn-group'>
                <a href="{{url('debtors/recommends')}}"
                   class="btn {{($recommends_count > 0) ? 'btn-danger' : 'btn-default'}}"{{($recommends_count > 0) ? '' : ' disabled'}}>Рекомендации
                    ({{$recommends_count}})</a>
                <a href="##" class="btn btn-default" onclick="$.debtorsCtrl.debtorsTotalPlanned({{$user->id}})">Общее
                    количество запланированных</a>
                <a href="{{url('debtors/forgotten')}}" class="btn btn-default" target="_blank">"Забытые" должники</a>
                <input type="button" class="btn btn-default" value="Зачет оплат" data-target='#userPaymentsModal'
                       data-toggle='modal'/>
                ...
                @if ($user->hasRole('debtors_chief') && $user->hasRole('debtors_personal'))
                    <a href="{{url('debtors/recurrent/massquerytask?str_podr=000000000007')}}" class="btn btn-default">Массовое
                        списание</a>
                @endif
                @if ($user->hasRole('debtors_chief') && $user->hasRole('debtors_remote'))
                    <a href="{{url('debtors/recurrent/massquerytask?str_podr=000000000006')}}" class="btn btn-default">Массовое
                        списание</a>
                    <a href="{{url('debtors/recurrent/massquerytask?str_podr=000000000006-1')}}"
                       class="btn btn-default">Масс. списание Ведущий</a>
                @endif
                ...
                <ul class="dropdown-menu">
                    ...
                    <li><a href="#" data-toggle="modal" data-target="#debtorsSiteLoginReportModal">Должник на сайте</a>
                    </li>
        ...
        <div class="col-xs-12 col-lg-5">
            <div class='panel panel-default'>
                <div class='panel-heading'>
                    <h3 class='panel-title'>Мероприятия
                        <button type="button" class="btn btn-default pull-right btn-xs" data-toggle="modal"
                                data-target="#debtorEventsSearchModal"><span class='glyphicon glyphicon-search'></span>
                            Поиск
                        </button>
                        &nbsp;
                        <a target="_blank" class="btn btn-default pull-right btn-xs"
                           onclick="$.debtorsCtrl.debtorsEventsToExcel(); return false;"><span
                                    class='glyphicon glyphicon-export'></span> В Excel</a>
                    </h3>
                </div>
            ...
            <div class='panel panel-default'>
                <div class='panel-heading'>
                    <h3 class='panel-title'>Должники
                        <button type="button" class="btn btn-default pull-right btn-xs" data-toggle="modal"
                                data-target="#debtorsSearchModal"><span class='glyphicon glyphicon-search'></span> Поиск
                        </button>
                        &nbsp;
                        <a target="_blank" class="btn btn-default pull-right btn-xs"
                           onclick="$.debtorsCtrl.debtorsToExcel(); return false;"><span
                                    class='glyphicon glyphicon-export'></span> В Excel</a>
                    </h3>
                </div>
                <div class='panel-body'>
    ...
    <div class="modal fade" id="debtorsSearchModal" tabindex="-1" role="dialog"
         aria-labelledby="debtorsSearchModalLabel" aria-hidden="true">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if (
                                                ...
                                                <select class='form-control'
                                                    ...
                                                    <option value=">">></option>
                                                    <option value=">=">>=</option>
                                                    <option value="like" {{($dsf['name'] == 'passports@fact_address_city') ? 'selected' : ''}}>
                                                        подобно
                                                    </option>
                                                </select>
                                            @endif
                                        ...
                                        <td>
                                            @if(array_key_exists('hidden_value_field',$dsf))
                                                <input name='{{$dsf['name']}}' type='{{$dsf['input_type']}}'
                                                       class='form-control autocomplete'
                                                       data-hidden-value-field='{{'search_field_'.$dsf['hidden_value_field']}}'/>
                                                <input name='{{'search_field_'.$dsf['hidden_value_field']}}'
                                                       type='hidden'/>
                                            @else
                                                <input name='{{'search_field_'.$dsf['name']}}'
                                                       type='{{$dsf['input_type']}}' class='form-control'/>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
