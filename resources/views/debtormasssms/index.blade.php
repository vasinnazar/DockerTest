@extends('app')
@section('title') Массовая рассылка SMS @stop
@section('css')
<style>
    .debtors-frame{
        height: 250px;
...
<div class='row'>
    <div class="col-xs-12">
        <button id="smsFilter" type="button" class="btn btn-default" data-toggle="modal" data-target="#debtorMassSmsFilterModal"><span class='glyphicon glyphicon-search'></span> Фильтр</button>
    </div>
</div>
<div class='row'>
    <div class="col-xs-12">
        <table id="debtorTransferAction" class="pull-right">
            <tr>
                <td style="padding-left: 20px; padding-right: 17px;">
                    <button id="smsTpls" class="btn btn-primary" data-toggle="modal" data-target="#debtorMassSMS">Шаблон SMS</button>
                </td>
                <td style="padding-left: 20px; padding-right: 17px;">
...
<div class="modal fade" id="debtorMassSmsFilterModal" tabindex="-1" role="dialog" aria-labelledby="debtorTransferFilterModalLabel" aria-hidden="true">
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.error == 'success') {
                    ...
                    $('#smsInfo').text('Ошибка: ' + data.error);
                }
            }
        });
    });
