@if($loans)
    ...
    @if ($loans['current_loan_id_1c'] != $id_1c)
        @if ($mLoan['has_result'] == 0)
            <a href="{{ url('/debtors/debtorcard/' . $mLoan['debtor_id']) }}" class='btn btn-danger' target="_blank"
               style="width: 100%; margin-bottom: 5px; font-size: 80%;">Некорректная инф. для {{ $id_1c }}</a>
            <br>
        @else
            <a href="{{ ($mLoan['debtor_id'] == 0) ? 'javascript:void(0);' : url('/debtors/debtorcard/' . $mLoan['debtor_id']) }}"
               class='btn btn-{{ ($mLoan['debt'] == 0 || $mLoan['debtor_id'] == 0) ? 'success' : 'danger' }}'
               target="_blank" style="width: 100%; margin-bottom: 5px; font-size: 80%;">№ {{ $id_1c }}
                от {{ date('d.m.Y', strtotime($mLoan['created_at'])) }} г.
                [ {{ ($mLoan['debt'] == 0) ? 'закрыт' : number_format($mLoan['debt'] / 100, '2', '.', '') . ' руб.' }} ]
                &nbsp;&nbsp; <span class="glyphicon glyphicon-time"
                                   aria-hidden="true"></span> {{ $mLoan['exp_days'] }} {{ $mLoan['responsible_user_id_1c'] }}
            </a>
            <br>
        @endif
    @endif
